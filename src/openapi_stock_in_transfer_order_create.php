<?php

declare(strict_types=1);
/**
 * This file is part of WangDian PHP SDK.
 *
 * @link     https://github.com/westng/wangdian-sdk-php
 * @document https://github.com/westng/wangdian-sdk-php
 * @contact  westng
 * @license  https://github.com/westng/wangdian-sdk-php/blob/main/LICENSE
 */
header('Content-Type: text/html; charset=UTF-8');
date_default_timezone_set('Asia/Shanghai');
require_once 'wdtsdk.php';

$sid = 'wdterp30';
$appkey = 'spw001';
$appsecret = 'aa:cc';
$service_url = 'http://172.17.105.65:30000/';

$client = new WdtErpClient($service_url, $sid, $appkey, $appsecret);

$transferInOrder = new stdClass();
$transferInOrder->src_order_no = 'TF202007080001';
$transferInOrder->warehouse_no = 'lich0313';
// $transferInOrder->logisctics_code='';
$transferInOrder->remark = 'api调拨入库';

$transferInOrderList = [];
$transferInOrderDetail1 = new stdClass();
$transferInOrderDetail1->spec_no = 'sjbm001';
$transferInOrderDetail1->num = 1;
$transferInOrderDetail1->unit_name = '瓶';
$transferInOrderDetail1->remark = 'api调拨入库';

array_push($transferInOrderList, $transferInOrderDetail1);

$data = $client->call('wms.stockin.Transfer.createOrder', $transferInOrder, $transferInOrderList, true);
echo json_encode($data, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT);
