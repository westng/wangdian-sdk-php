...
/**
 * 仓库查询测试
 * SettingWarehouseQueryWarehouse.
 */
class SettingWarehouseQueryWarehouse
{
    public static function run(): void
    {
        ...
        try {
            ...
            $parMap = new stdClass();
            // 可以添加查询条件，例如：
            // $parMap->warehouse_no = 'your_warehouse_no';
            // $parMap->warehouse_name = 'your_warehouse_name';

            echo "🔍 开始查询仓库信息...\n";

            // 调用API
            $result = $client->pageCall('setting.Warehouse.queryWarehouse', $pager, $parMap);
            ...
            if ($result) {
                ...
                if (isset($result->data, $result->data->details) && ! empty($result->data->details)) {
                    echo "🏭 仓库列表:\n";
                    if (is_array($result->data->details)) {
                        foreach ($result->data->details as $index => $warehouse) {
                            echo '  ' . ($index + 1) . '. 仓库编号: ' . ($warehouse->warehouse_no ?? 'N/A')
                                 . ', 仓库名称: ' . ($warehouse->warehouse_name ?? 'N/A')
                                 . ', 状态: ' . ($warehouse->status ?? 'N/A') . "\n";
                        }
                    } else {
...
if (php_sapi_name() === 'cli') {
    SettingWarehouseQueryWarehouse::run();
}
